...
def validate_xacro_file(file_path: Path):
    ...
    try:
        ...
        print("✓ File is well-formed XML")
        print(f"✓ Root element: {root.tag}")

        # Check for common xacro elements
        if root.tag == "robot":
            print("✓ Root element is 'robot' (correct for URDF/xacro)")
        ...
        if "xacro" in root.attrib.get("xmlns:xacro", ""):
            print("✓ Xacro namespace declared")
        else:
            # Check if any xacro elements exist
            if includes or macros:
                print(
                    "⚠ Warning: Xacro elements found but namespace might not be properly declared"
                )
        ...
        print("For full xacro processing (macro expansion, property substitution),")
        print("you need ROS installed: sudo apt install ros-<distro>-xacro")
