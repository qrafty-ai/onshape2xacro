...
def convert_xacro_to_urdf(
    xacro_path: Path, output_urdf: Path, fix_paths: bool = True
) -> bool:
    ...
    try:
        ...
        print(
            "  Note: For full xacro support, install ROS: sudo apt install ros-<distro>-xacro"
        )
        return True
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Convert xacro file to URDF file")
    parser.add_argument("xacro_file", type=Path, help="Path to xacro file")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
