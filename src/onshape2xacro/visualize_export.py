...
def build_robot_structure(output_dir: Path) -> Dict:
    ...
    for mesh_file in mesh_files:
        ...
        for link_name in all_links.keys():
            if mesh_name.lower().replace("_", "").replace("-", "") in link_name.lower().replace("_", "").replace("-", ""):
...
def print_text_visualization(structure: Dict, output_dir: Path):
    ...
    def print_tree(link_name: str, prefix: str = "", is_last: bool = True):
        ...
        for i, (child, joint_name, joint_type) in enumerate(children):
            ...
            print_tree(child, prefix + extension + ("    " if is_last_child else "│   "), is_last_child)
    ...
    print(f"  Links with Meshes: {sum(1 for l in structure['links'].values() if l['has_mesh'])}")
    print(f"  Links without Meshes: {sum(1 for l in structure['links'].values() if not l['has_mesh'])}")
...
def create_graph_visualization(structure: Dict, output_path: Path):
    ...
    root_links = [l for l in links if l not in all_children]
    ...
    for link, level in y_levels.items():
        ...
        index = sorted([l for l, lvl in y_levels.items() if lvl == level]).index(link)
    ...
    for link_name, (x, y) in positions.items():
        ...
        # Add label
        ax.text(x, y - 0.05, link_name, ha="center", va="top", 
    ...
    for joint in joints:
        ...
        if parent in positions and child in positions:
            ...
            arrow = FancyArrowPatch((x1, y1), (x2, y2),
                                   ...
                                   color="#2196F3", linewidth=2, zorder=1)
            ax.add_patch(arrow)
            
            # Add joint label
            mid_x, mid_y = (x1 + x2) / 2, (y1 + y2) / 2
            ax.text(mid_x, mid_y + 0.02, joint["name"], 
